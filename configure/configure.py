    ...
    while True:
        ...
        if command == 0:
            ...
            while True:
                ...
                for wifi in wifis:
                    index += 1
                    print "[{0}]: Edit/delete ssid `{1}'".format(index,
                            wifi.get_ssid())

                add_command = index + 1

                print '[{0}]: Add new Wifi access point'.format(add_command)
                ...
                if command == 0:
                    ...
                    ssid = raw_input('SSID (i.e. access point name): '
                            ).strip()
                    psk = raw_input('Access Point Password: ').strip()
                    ...
                    while True:
                        print ''
                        print "Edit WiFi '{0}' access point".format(wifi.get_ssid())
                        print '[0] Done editing this Wifi access point'
                        print "[1] Edit WiFi Name (currently '{0})'".format(wifi.get_ssid())
                        print '[2] Change Wifi Password'
                        print "[3] Delete entire '{0}' access point)".format(wifi.get_ssid())
                        try:
                        ...
                        if command == 0:
                            ...
                            wifi.set_basic_settings(wifi.get_ssid(),
                                    psk=psk)
                        elif command == 3:
            ...
            for line in lines:
                columns = line.split()
                if len(columns) >= 7 and columns[2] == 'IPv4' \
                    and columns[-2] == 'Workstation':
            ...
            while True:
                ...
                for machine_name in sorted_machine_names:
                    print '[{0}]: Set up SSH keys for {1}'.format(index,
                            machine_name)
                    index += 1
                ...
                if command == 0:
                    ...
                    who_am_i_process = subprocess.Popen(['whoami'],
                            stdout=subprocess.PIPE)
                    local_user_name = \
                        who_am_i_process.communicate()[0].strip()
                    ...
                    user_name = \
                        raw_input('User name on {0}.local [{1}]: '.format(machine_name,
                                  default_user_name)).strip()
                    if user_name == '':
                    ...
                    command = ['ssh-copy-id',
                               '{0}@{1}.local'.format(user_name,
                               machine_name)]
                    ssh_copy_id = subprocess.Popen(command, stdin=None,
            ...
            if old_hostname != new_hostname:
                ...
                returncode = subprocess.call(['/usr/bin/sudo', file_dir
                        + '/change_hostname.py', hostname_file_name,
    ...
    def __init__(self, connection=None):
        ...
        if connection != None:
            if connection.GetSettings()['connection']['type'] \
                == '802-11-wireless':
                self.ssid = connection.GetSettings()['802-11-wireless'
                        ]['ssid']
                self.uuid = connection.GetSettings()['connection'
                        ]['uuid']
    ...
    def save(self):
        if self.connection != None:
            ...
            try:
                ...
                sys.exit(0)
            self.connection = \
                NetworkManager.Settings.GetConnectionByUuid(self.uuid)
